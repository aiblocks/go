...
import (
	...
	sdk "github.com/aiblocks/go/clients/millenniumclient"
	"github.com/aiblocks/go/clients/aiblockscore"
	"github.com/aiblocks/go/keypair"
	proto "github.com/aiblocks/go/protocols/millennium"
...
const (
	...
	aiblocksCorePostgresPassword = "integration-tests-password"
	adminPort                   = 6060
)

var (
	aiblocksCorePort         = mustPort("tcp", "11626")
	aiblocksCorePostgresPort = mustPort("tcp", "5432")
...
func NewTest(t *testing.T, config Config) *Test {
	if os.Getenv("MILLENNIUM_INTEGRATION_TESTS") == "" {
	...
	image := "aiblocks/quickstart:testing"
	skipCreation := os.Getenv("MILLENNIUM_SKIP_CREATION") != ""
	...
	if skipCreation {
		...
		if i.container.ID != "" {
			...
			os.Unsetenv("MILLENNIUM_SKIP_CREATION")
			skipCreation = false
		}
	}
	...
	aiblocksCoreBinding := containerInfo.NetworkSettings.Ports[aiblocksCorePort][0]
	coreURL := fmt.Sprintf("http://%s:%s", aiblocksCoreBinding.HostIP, aiblocksCoreBinding.HostPort)
	// only use millennium from quickstart container when testing captive core
	if os.Getenv("MILLENNIUM_INTEGRATION_ENABLE_CAPTIVE_CORE") == "" {
		i.startMillennium(containerInfo, coreURL)
	}

	doCleanup = false
	i.hclient = &sdk.Client{MillenniumURL: "http://localhost:8000"}
	i.cclient = &aiblockscore.Client{URL: coreURL}
...
func (i *Test) setupMillenniumBinary() {
	// only use millennium from quickstart container when testing captive core
	if os.Getenv("MILLENNIUM_INTEGRATION_ENABLE_CAPTIVE_CORE") == "" {
		return
	}

	if os.Getenv("MILLENNIUM_BIN_DIR") == "" {
		i.t.Fatal("MILLENNIUM_BIN_DIR env variable not set")
	}

	millenniumBinaryContents, err := ioutil.ReadFile(os.Getenv("MILLENNIUM_BIN_DIR") + "/millennium")
	if err != nil {
		i.t.Fatal(errors.Wrap(err, "error reading millennium binary file"))
	}

	// Create a tar archive with millennium binary (required by docker API).
	var buf bytes.Buffer
	tw := tar.NewWriter(&buf)
	hdr := &tar.Header{
		Name: "aiblocks-millennium",
		Mode: 0755,
		Size: int64(len(millenniumBinaryContents)),
	}
	if err = tw.WriteHeader(hdr); err != nil {
		i.t.Fatal(errors.Wrap(err, "error writing tar header"))
	}
	if _, err = tw.Write(millenniumBinaryContents); err != nil {
	...
	i.t.Log("Copying custom millennium binary...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	err = i.cli.CopyToContainer(ctx, i.container.ID, "/usr/bin/", &buf, types.CopyToContainerOptions{})
	if err != nil {
		i.t.Fatal(errors.Wrap(err, "error copying custom millennium binary"))
	}
}

func (i *Test) startMillennium(containerInfo types.ContainerJSON, coreURL string) {
	aiblocksCorePostgres := containerInfo.NetworkSettings.Ports[aiblocksCorePostgresPort][0]
	aiblocksCorePostgresURL := fmt.Sprintf(
		"postgres://aiblocks:%s@%s:%s/core",
		aiblocksCorePostgresPassword,
		aiblocksCorePostgres.HostIP,
		aiblocksCorePostgres.HostPort,
	)

	historyArchive := containerInfo.NetworkSettings.Ports[historyArchivePort][0]

	millenniumPostgresURL := dbtest.Postgres(i.t).DSN

	config, configOpts := millennium.Flags()
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			i.app = millennium.NewAppFromFlags(config, configOpts)
		},
	}
	cmd.SetArgs([]string{
		"--aiblocks-core-url",
		...
		"--ingest",
		"--db-url",
		millenniumPostgresURL,
		"--aiblocks-core-db-url",
		aiblocksCorePostgresURL,
		"--network-passphrase",
		NetworkPassphrase,
		"--apply-migrations",
		"--admin-port",
	...
	if err := cmd.Execute(); err != nil {
		i.t.Fatalf("cannot initialize millennium: %s", err)
	}
...
func (i *Test) waitForIngestionAndUpgrade() {
	for t := 30 * time.Second; t >= 0; t -= time.Second {
		...
		// Millennium not running. We ensure that's is up and correct by checking
		// the root response.
		if root.IngestSequence > 0 &&
			root.MillenniumSequence > 0 &&
			root.CurrentProtocolVersion == i.config.ProtocolVersion {
			i.t.Log("Millennium ingesting and protocol version matches...")
			return
		}
		time.Sleep(time.Second)
	}

	i.t.Fatal("Millennium not ingesting...")
}

// Client returns millennium.Client connected to started Millennium instance.
func (i *Test) Client() *sdk.Client {
	return i.hclient
}

// LedgerIngested returns true if the ledger with a given sequence has been
// ingested by Millennium. Panics in case of errors.
...
// closed by AiBlocks-Core. Panics in case of errors. Note it's different
// than LedgerIngested because it checks if the ledger was closed, not
// necessarily ingested (ex. when rebuilding state Millennium does not ingest
// recent ledgers).
...
// AdminPort returns Millennium admin port.
...
func createTestContainer(i *Test, image string) error {
	...
	if err != nil {
		...
		args.Add("reference", "aiblocks/quickstart:testing")
	...
	if os.Getenv("MILLENNIUM_INTEGRATION_ENABLE_CAPTIVE_CORE") != "" {
		containerConfig.Env = append(containerConfig.Env,
			...
			"AIBLOCKS_CORE_BINARY_PATH=/opt/aiblocks/core/bin/start",
			"AIBLOCKS_CORE_CONFIG_PATH=/opt/aiblocks/core/etc/aiblocks-core.cfg",
		)
		...
		containerConfig.Env = append(containerConfig.Env,
			"POSTGRES_PASSWORD="+aiblocksCorePostgresPassword,
		)
		containerConfig.ExposedPorts = nat.PortSet{
			aiblocksCorePort:         struct{}{},
			aiblocksCorePostgresPort: struct{}{},
...
func (i *Test) CreateAccounts(count int, initialBalance string) ([]*keypair.Full, []txnbuild.Account) {
	...
	for _, keys := range pairs {
		i.t.Logf("Funded %s (%s) with %s DLO.\n",
...
func (i *Test) SubmitTransactionXDR(txb64 string) (proto.Transaction, error) {
	...
	// We need to close the ledger in parallel because Millennium's submission endpoint
	// blocks until the transaction is in a closed ledger
	go func() {
		// This sleep is ugly, but a better approach would probably require
		// instrumenting Millennium to tell us when the transaction was sent to core.
...
func (i *Test) LogFailedTx(txResponse proto.Transaction, millenniumResult error) {
	t := i.CurrentTest()
	assert.NoErrorf(t, millenniumResult, "Submitting the transaction failed")
	if prob := sdk.GetError(millenniumResult); prob != nil {
		t.Logf("  problem: %s\n", prob.Problem.Detail)
...
func (i *Test) RunMillenniumCLICommand(cmd ...string) {
	fullCmd := append([]string{"/aiblocks/millennium/bin/millennium"}, cmd...)
