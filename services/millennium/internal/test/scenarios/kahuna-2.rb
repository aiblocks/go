...
# one-time signer being consumed
  ...
  key = AiBlocks::SignerKey.hash_x(x)
  set_options :onetime, signer: AiBlocks::Signer.new({
    key: key, 
    weight: 1,
  })
  close_ledger #4
  ...
  tx = AiBlocks::Transaction.manage_data({
    account:  account,
  ...
  env = tx.to_envelope
  env.signatures << AiBlocks::DecoratedSignature.new({
