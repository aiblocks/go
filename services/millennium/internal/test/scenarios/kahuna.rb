...
# aiblocks's features, exercising each feature and option at least once.
#
# As new features are added to aiblocks, this scenario will get updated to
# exercise those new features.  This scenario is used during the millennium
# ingestion tests.
#
use_manual_close
KP = AiBlocks::KeyPair
...
# time bounds
  ...
  create_account :time_bounds do |env|
    env.tx.time_bounds = AiBlocks::TimeBounds.new(min_time: 100, max_time: Time.parse("2020-01-01").to_i)
...
# different source account
  ...
  payment :master, :different_source,  [:native, "10.00"] do |env|
    newop = AiBlocks::Operation.from_xdr env.tx.operations[0].to_xdr
