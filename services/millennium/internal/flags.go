...
import (
	...
	apkg "github.com/aiblocks/go/support/app"
...
func applyMigrations(config Config) {
	...
	if err != nil {
		stdLog.Fatalf("could not connect to millennium db: %v", err)
	}
	defer dbConn.Close()
	...
	if numMigrations > 0 {
		stdLog.Printf("successfully applied %v millennium migrations\n", numMigrations)
	}
}
...
func checkMigrations(config Config) {
	...
	if len(migrationsToApplyUp) > 0 {
		...
		stdLog.Printf("A database migration is required to run this version (%v) of Millennium. Run \"millennium db migrate up\" to update your DB. Consult the Changelog (https://github.com/aiblocks/go/blob/master/services/millennium/CHANGELOG.md) for more information.", apkg.Version())
		os.Exit(1)
	}

	nMigrationsDown := schema.GetNumMigrationsDown(config.DatabaseURL)
	if nMigrationsDown > 0 {
		stdLog.Printf("A database migration DOWN to an earlier version of the schema is required to run this version (%v) of Millennium. Consult the Changelog (https://github.com/aiblocks/go/blob/master/services/millennium/CHANGELOG.md) for more information.", apkg.Version())
		stdLog.Printf("In order to migrate the database DOWN, using the HIGHEST version number of Millennium you have installed (not this binary), run \"millennium db migrate down %v\".", nMigrationsDown)
		os.Exit(1)
	}
}
...
func Flags() (*Config, support.ConfigOptions) {
	...
	// flags defines the complete flag configuration for millennium.
	// Add a new entry here to connect a new field in the millennium.Config struct
	var flags = support.ConfigOptions{
		&support.ConfigOption{
			...
			Required:  true,
			Usage:     "millennium postgres database to connect with",
		},
		&support.ConfigOption{
			...
			FlagDefault: "",
			Required:    false,
			Usage:       "path to aiblocks core binary (--remote-captive-core-url has higher precedence)",
			ConfigKey:   &config.AiBlocksCoreBinaryPath,
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			ConfigKey:   &config.AiBlocksCoreConfigPath,
		},
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "[experimental flag!] causes Millennium to ingest from a AiBlocks Core subprocess instead of a persistent AiBlocks Core database",
			ConfigKey:   &config.EnableCaptiveCoreIngestion,
		},
		&support.ConfigOption{
			Name:      AiBlocksCoreDBURLFlagName,
			EnvVar:    "AIBLOCKS_CORE_DATABASE_URL",
			ConfigKey: &config.AiBlocksCoreDatabaseURL,
			OptType:   types.String,
			Required:  false,
			Usage:     "aiblocks-core postgres database to connect with",
		},
		&support.ConfigOption{
			Name:      AiBlocksCoreURLFlagName,
			ConfigKey: &config.AiBlocksCoreURL,
			OptType:   types.String,
			Usage:     "aiblocks-core to connect with (for http commands)",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			OptType:     types.Int,
			FlagDefault: 0,
			Usage:       "when set has a priority over millennium-db-max-open-connections, millennium-db-max-idle-connections. max millennium database open connections may need to be increased when responses are slow but DB CPU is normal",
		},
		&support.ConfigOption{
			Name:        "millennium-db-max-open-connections",
			ConfigKey:   &config.MillenniumDBMaxOpenConnections,
			OptType:     types.Int,
			FlagDefault: 20,
			Usage:       "max millennium database open connections. may need to be increased when responses are slow but DB CPU is normal",
		},
		&support.ConfigOption{
			Name:        "millennium-db-max-idle-connections",
			ConfigKey:   &config.MillenniumDBMaxIdleConnections,
			OptType:     types.Int,
			FlagDefault: 20,
			Usage:       "max millennium database idle connections. may need to be set to the same value as millennium-db-max-open-connections when responses are slow and DB CPU is normal, because it may indicate that a lot of time is spent closing/opening idle connections. This can happen in case of high variance in number of requests. must be equal or lower than max open connections",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			FlagDefault:    55,
			CustomSetValue: support.SetDuration,
			Usage:          "defines the timeout of connection after which 504 response will be sent or stream will be closed, if Millennium is behind a load balancer with idle connection timeout, this should be set to a few seconds less that idle timeout, does not apply to POST /transactions",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{ // Action needed in release: millennium-v2.0.0
		...
		&support.ConfigOption{
			...
			OptType:   types.String,
			Usage:     "TLS certificate file to use for securing connections to millennium",
		},
		&support.ConfigOption{
			Name:      "tls-key",
			ConfigKey: &config.TLSKey,
			OptType:   types.String,
			Usage:     "TLS private key file to use for securing connections to millennium",
		},
		&support.ConfigOption{
			Name:        "ingest",
			...
			OptType:     types.Bool,
			FlagDefault: false,
			Usage:       "causes this millennium process to ingest data from aiblocks-core into millennium's db",
		},
		&support.ConfigOption{
			Name:        "cursor-name",
			EnvVar:      "CURSOR_NAME",
			...
			OptType:     types.String,
			FlagDefault: "MILLENNIUM",
			Usage:       "ingestor cursor used by millennium to ingest from aiblocks core. must be uppercase and unique for each millennium instance ingesting from that core instance.",
		},
		&support.ConfigOption{
			...
			OptType:     types.Uint,
			FlagDefault: uint(0),
			Usage:       "the minimum number of ledgers to maintain within millennium's history tables.  0 signifies an unlimited number of ledgers will be retained",
		},
		&support.ConfigOption{
			...
			OptType:     types.Uint,
			FlagDefault: uint(0),
			Usage:       "the maximum number of ledgers the history db is allowed to be out of date from the connected aiblocks-core db before millennium considers history stale",
		},
		&support.ConfigOption{
			...
			OptType:     types.Bool,
			FlagDefault: false,
			Usage:       "causes the ingester to skip reporting the last imported ledger state to aiblocks-core",
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "applies pending migrations before starting millennium",
		},
	}
...
func NewAppFromFlags(config *Config, flags support.ConfigOptions) *App {
	...
	if config.AiBlocksCoreURL == "" {
		log.Fatalf("flag --%s cannot be empty", AiBlocksCoreURLFlagName)
	}
	if config.Ingest && !config.EnableCaptiveCoreIngestion && config.AiBlocksCoreDatabaseURL == "" {
		log.Fatalf("flag --%s cannot be empty", AiBlocksCoreDBURLFlagName)
	}
...
func ApplyFlags(config *Config, flags support.ConfigOptions) {
	...
	if config.EnableCaptiveCoreIngestion {
		binaryPath := viper.GetString("aiblocks-core-binary-path")
		remoteURL := viper.GetString("remote-captive-core-url")
		if binaryPath == "" && remoteURL == "" {
			stdLog.Fatalf("Invalid config: captive core requires that either --aiblocks-core-binary-path or --remote-captive-core-url is set")
		}
	}
	if config.Ingest {
		// When running live ingestion a config file is required too
		validateBothOrNeither("aiblocks-core-binary-path", "aiblocks-core-config-path")
	}
	...
	if config.MaxDBConnections != 0 {
		config.MillenniumDBMaxOpenConnections = config.MaxDBConnections
		config.MillenniumDBMaxIdleConnections = config.MaxDBConnections
	}
}
