...
import (
	...
	"github.com/aiblocks/go/exp/orderbook"
	...
	"github.com/aiblocks/go/services/millennium/internal/simplepath"
	"github.com/aiblocks/go/services/millennium/internal/txsub"
	"github.com/aiblocks/go/services/millennium/internal/txsub/sequence"
...
func mustInitMillenniumDB(app *App) {
	maxIdle := app.config.MillenniumDBMaxIdleConnections
	maxOpen := app.config.MillenniumDBMaxOpenConnections
	if app.config.Ingest {
		maxIdle -= ingest.MaxDBConnections
		maxOpen -= ingest.MaxDBConnections
		if maxIdle <= 0 {
			log.Fatalf("max idle connections to millennium db must be greater than %d", ingest.MaxDBConnections)
		}
		if maxOpen <= 0 {
			log.Fatalf("max open connections to millennium db must be greater than %d", ingest.MaxDBConnections)
		}
	}
...
func initExpIngester(app *App) {
	...
	if !app.config.EnableCaptiveCoreIngestion {
		coreSession = mustNewDBSession(app.config.AiBlocksCoreDatabaseURL, ingest.MaxDBConnections, ingest.MaxDBConnections)
	}
	app.ingester, err = ingest.NewSystem(ingest.Config{
		...
		AiBlocksCoreURL:           app.config.AiBlocksCoreURL,
		AiBlocksCoreCursor:        app.config.CursorName,
		AiBlocksCoreBinaryPath:    app.config.AiBlocksCoreBinaryPath,
		AiBlocksCoreConfigPath:    app.config.AiBlocksCoreConfigPath,
...
func initPathFinder(app *App) {
	...
	app.orderBookStream = ingest.NewOrderBookStream(
		&history.Q{app.MillenniumSession(app.ctx)},
		orderBookGraph,
	)
...
func initDbMetrics(app *App) {
	...
	app.historyElderLedgerCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{Namespace: "millennium", Subsystem: "history", Name: "elder_ledger"},
		func() float64 {
			ls := ledger.CurrentState()
	...
	app.coreLatestLedgerCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{Namespace: "millennium", Subsystem: "aiblocks_core", Name: "latest_ledger"},
		func() float64 {
			ls := ledger.CurrentState()
	...
	app.dbMaxOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "millennium", Subsystem: "db", Name: "max_open_connections"},
		func() float64 {
			// Right now MaxOpenConnections in Millennium is static however it's possible that
	...
	app.dbOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "millennium", Subsystem: "db", Name: "open_connections"},
		func() float64 {
	...
	app.dbInUseConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "millennium", Subsystem: "db", Name: "in_use_connections"},
		func() float64 {
	...
	app.dbWaitCountCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "millennium", Subsystem: "db", Name: "wait_count_total",
	...
	app.dbWaitDurationCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "millennium", Subsystem: "db", Name: "wait_duration_seconds_total",
...
func initSubmissionSystem(app *App) {
	app.submitter = &txsub.System{
		...
		Submitter:       txsub.NewDefaultSubmitter(http.DefaultClient, app.config.AiBlocksCoreURL),
		SubmissionQueue: sequence.NewManager(),
		DB: func(ctx context.Context) txsub.MillenniumDB {
			return &history.Q{Session: app.MillenniumSession(ctx)}
		},
	}
}
