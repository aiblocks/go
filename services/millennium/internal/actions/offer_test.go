...
func TestGetOfferByIDHandler(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			func(response interface{}) {
				offer := response.(millennium.Offer)
...
func pageableToOffers(t *testing.T, page []hal.Pageable) []millennium.Offer {
	var offers []millennium.Offer
	for _, entry := range page {
		offers = append(offers, entry.(millennium.Offer))
	}
	return offers
}
