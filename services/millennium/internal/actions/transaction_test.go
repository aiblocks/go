...
func checkOuterHashResponse(
	...
	transactionResponse millennium.Transaction,
) {
...
func TestFeeBumpTransactionPage(t *testing.T) {
	...
	feeBumpResponse := records[0].(millennium.Transaction)
	checkOuterHashResponse(tt, fixture, feeBumpResponse)

	normalTxResponse := records[1].(millennium.Transaction)
...
func TestFeeBumpTransactionResource(t *testing.T) {
	...
	tt.Assert.NoError(err)
	byOuterHash := resource.(millennium.Transaction)
	...
	byInnerHash := resource.(millennium.Transaction)
