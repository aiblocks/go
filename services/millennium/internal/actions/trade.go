...
import (
	...
	"github.com/aiblocks/go/services/millennium/internal/context"
	...
	"github.com/aiblocks/go/support/time"
...
func (handler GetTradeAggregationsHandler) GetResource(w HeaderWriter, r *http.Request) (interface{}, error) {
	...
	aggregations := []millennium.TradeAggregation{}
	for _, record := range records {
		var res millennium.TradeAggregation
...
func (handler GetTradeAggregationsHandler) buildPage(r *http.Request, records []millennium.TradeAggregation) (hal.Page, error) {
	ctx := r.Context()
