...
import (
	...
	"github.com/aiblocks/go/services/millennium/internal/render/problem"
	"github.com/aiblocks/go/services/millennium/internal/test"
	supportProblem "github.com/aiblocks/go/support/render/problem"
)
...
func TestGetOperationsWithoutFilter(t *testing.T) {
	...
	q := &history.Q{tt.MillenniumSession()}
...
func TestGetOperationsIncludeFailed(t *testing.T) {
	...
	// NULL value
	_, err = tt.MillenniumSession().ExecRaw(
...
func TestGetOperation(t *testing.T) {
	...
	record, err := handler.GetResource(
		httptest.NewRecorder(),
		makeRequest(
			t, map[string]string{}, map[string]string{"id": "8589938689"}, tt.MillenniumSession(),
		),
	)
	tt.Assert.NoError(err)
	op := record.(operations.Operation)
	...
	_, err = handler.GetResource(
		httptest.NewRecorder(),
		makeRequest(
			t, map[string]string{}, map[string]string{"id": "9589938689"}, tt.MillenniumSession(),
		),
	)
	...
	_, err = handler.GetResource(
		httptest.NewRecorder(),
		makeRequest(
			t, map[string]string{}, map[string]string{"id": "0"}, tt.MillenniumSession(),
		),
	)
...
func TestOperation_IncludeTransaction(t *testing.T) {
	...
	record, err := handler.GetResource(
		httptest.NewRecorder(),
		makeRequest(
			t, map[string]string{}, map[string]string{"id": "261993009153"}, tt.MillenniumSession(),
		),
	)
	...
	record, err = handler.GetResource(
		httptest.NewRecorder(),
		makeRequest(
			t, map[string]string{"join": "transactions"}, map[string]string{"id": "261993009153"}, tt.MillenniumSession(),
		),
	)
	op = record.(operations.BumpSequence)
	tt.Assert.NotNil(op.Transaction)
