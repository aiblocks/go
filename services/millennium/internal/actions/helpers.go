...
import (
	...
	"github.com/aiblocks/go/services/millennium/internal/assets"
...
// getAccountID retireves an xdr.AccountID by attempting to decode a aiblocks
...
func FullURL(ctx context.Context) *url.URL {
	url := millenniumContext.BaseURL(ctx)
	r := millenniumContext.RequestFromContext(ctx)
	if r != nil {
		url.Path = r.URL.Path
