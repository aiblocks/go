...
func TestAssetStats(t *testing.T) {
	...
	issuerFlags := millennium.AccountFlags{
		AuthRequired:  true,
		AuthImmutable: true,
	}
	...
	usdAssetStatResponse := millennium.AssetStat{
		Amount:      "0.0000001",
	...
	etherAssetStatResponse := millennium.AssetStat{
		Amount:      "0.0000023",
	...
	otherUSDAssetStatResponse := millennium.AssetStat{
		Amount:      "0.0000001",
	...
	otherUSDAssetStatResponse.Links.Toml = hal.NewLink(
		"https://" + otherIssuer.HomeDomain + "/.well-known/aiblocks.toml",
	)
	...
	eurAssetStatResponse := millennium.AssetStat{
		Amount:      "0.0000111",
	...
	for _, testCase := range []struct {
		name        string
		queryParams map[string]string
		expected    []millennium.AssetStat
	}{
		{
			"default parameters",
			map[string]string{},
			[]millennium.AssetStat{
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for i, item := range results {
				assetStat := item.(millennium.AssetStat)
...
func TestAssetStatsIssuerDoesNotExist(t *testing.T) {
	...
	expectedAssetStatResponse := millennium.AssetStat{
		Amount:      "0.0000001",
		NumAccounts: usdAssetStat.NumAccounts,
		Asset: base.Asset{
			Type:   "credit_alphanum4",
	...
	assetStat := results[0].(millennium.AssetStat)
