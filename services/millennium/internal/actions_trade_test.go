...
import (
	...
	. "github.com/aiblocks/go/services/millennium/internal/db2/history"
	. "github.com/aiblocks/go/services/millennium/internal/test/trades"
	"github.com/aiblocks/go/support/render/hal"
	aiblocksTime "github.com/aiblocks/go/support/time"
...
func TestTradeActions_Index(t *testing.T) {
	...
	defer ht.Finish()
	var records []millennium.Trade
	var firstTrade millennium.Trade
	...
	if ht.Assert.Equal(200, w.Code) {
		...
		// 	ensure created_at is populated correctly
		l := history.Ledger{}
		hq := history.Q{Session: ht.MillenniumSession()}
...
func testTradeAggregationPrices(t *HTTPT, record millennium.TradeAggregation) {
...
func TestTradeActions_Aggregation(t *testing.T) {
	...
	var records []millennium.TradeAggregation
	var record millennium.TradeAggregation
	var nextLink string
...
func TestTradeActions_AmountsExceedInt64(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		timestamp := aiblocksTime.MillisFromInt64(start + (minute * int64(i-1)))
		err := IngestTestTrade(
...
func TestTradeActions_IndexRegressions(t *testing.T) {
	t.Run("Regression:  https://github.com/aiblocks/go/services/millennium/internal/issues/318", func(t *testing.T) {
		ht := StartHTTPTest(t, "trades")
		defer ht.Finish()
	...
	t.Run("Regression for nil prices: https://github.com/aiblocks/go/issues/357", func(t *testing.T) {
		ht := StartHTTPTest(t, "trades")
		defer ht.Finish()
		...
		_ = ht.MillenniumDB.MustExec("UPDATE history_trades SET price_n = NULL, price_d = NULL")
		w = ht.Get("/trades")
...
// https://github.com/aiblocks/go/issues/215
