...
func TestEffectActions_Index(t *testing.T) {
	...
	t.Run("omnibus test", func(t *testing.T) {
		...
		// test streaming, regression for https://github.com/aiblocks/go/services/millennium/internal/issues/147
		...
		q := history.Q{ht.MillenniumSession()}
	...
	t.Run("Effect resource props", func(t *testing.T) {
		...
		if ht.Assert.Equal(200, w.Code) {
			...
			err := ht.MillenniumDB.Get(&ledger2, "SELECT * FROM history_ledgers WHERE sequence = 2")
...
func TestEffectsForFeeBumpTransaction(t *testing.T) {
	...
	defer ht.Finish()
	test.ResetMillenniumDB(t, ht.MillenniumDB)
	q := &history.Q{ht.MillenniumSession()}
