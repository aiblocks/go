...
When you submit a malformed transaction to Millennium, Millennium will return a `transaction_malformed`
...
As with all errors Millennium returns, `transaction_malformed` follows the
...
```json
{
  "type": "https://aiblocks.io/millennium-errors/transaction_malformed",
  "title": "Transaction Malformed",
  "status": 400,
  "detail": "Millennium could not decode the transaction envelope in this request. A transaction should be an XDR TransactionEnvelope struct encoded using base64.  The envelope read from this request is echoed in the `extras.envelope_xdr` field of this response for your convenience.",
  "extras": {
