...
If Millennium cannot understand a request due to invalid parameters, it will return a `bad_request`
...
As with all errors Millennium returns, `bad_request` follows the
...
```shell
$ curl -X GET "https://millennium-testnet.aiblocks.io/ledgers?limit=invalidlimit"
{
