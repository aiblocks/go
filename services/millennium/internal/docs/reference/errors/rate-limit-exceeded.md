...
When a single user makes too many requests to Millennium in a one hour time frame, Millennium returns a
`rate_limit_exceeded` error. By default, Millennium allows 3600 requests per hour -- an average of one
...
As with all errors Millennium returns, `rate_limit_exceeded` follows the
...
```json
{
  "type": "https://aiblocks.io/millennium-errors/rate_limit_exceeded",
  "title": "Rate Limit Exceeded",
  "status": 429,
