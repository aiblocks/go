...
replacement: https://developers.aiblocks.io/api/introduction/response-format/
---

Rather than using a fully custom way of representing the resources we expose in
Millennium, we use [HAL](http://stateless.co/hal_specification.html). HAL is a
...
The above response is for the genesis ledger of the AiBlocks test network, and
the links in the `_links` attribute provide links to other relavant resources in
Millennium.  Notice the object beneath the `transactions` key.  The key of each
...
Over time there will be millions of transactions in the AiBlocks network's ledger
