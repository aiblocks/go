...
Trades can also be caused by successful [path payments](https://www.aiblocks.io/developers/learn/concepts/exchange.html), because path payments involve fulfilling offers.
...
#### Synthetic Offer Ids
Offer ids in the millennium trade resource (base_offer_id, counter_offer_id) are synthetic and don't always reflect the respective aiblocks-core offer ids. This is due to the fact that aiblocks-core does not assign offer ids when an offer gets filled immediately. In these cases, Millennium synthetically generates an offer id for the buying offer, based on the total order id of the offer operation. This allows wallets to aggregate historical trades based on offer ids without adding special handling for edge cases. The exact encoding can be found [here](https://github.com/aiblocks/go/blob/master/services/millennium/internal/db2/history/synt_offer_id.go). 
