...
clientData:
  laboratoryUrl: https://www.aiblocks.io/laboratory/#explorer?resource=order_book&endpoint=details
replacement: https://developers.aiblocks.io/api/aggregations/order-books/
---

People on the AiBlocks network can make [offers](../resources/offer.md) to buy or sell assets.
These offers are summarized by the assets being bought and sold in
[orderbooks](../resources/orderbook.md).

Millennium will return, for each orderbook, a summary of the orderbook and the bids and asks
...
```sh
curl "https://millennium-testnet.aiblocks.io/order_book?selling_asset_type=native&buying_asset_type=credit_alphanum4&buying_asset_code=FOO&buying_asset_issuer=GBAUUA74H4XOQYRSOW2RZUA4QL5PB37U3JS5NE3RTB2ELJVMIF5RLMAG&limit=20"
```
...
server.orderbook(new AiBlocksSdk.Asset.native(), new AiBlocksSdk.Asset('FOO', 'GBAUUA74H4XOQYRSOW2RZUA4QL5PB37U3JS5NE3RTB2ELJVMIF5RLMAG'))
  .call()
  .then(function(resp) {
    console.log(resp);
  })
...
var es = server.orderbook(new AiBlocksSdk.Asset.native(), new AiBlocksSdk.Asset('FOO', 'GBAUUA74H4XOQYRSOW2RZUA4QL5PB37U3JS5NE3RTB2ELJVMIF5RLMAG'))
  .cursor('now')
  .stream({
