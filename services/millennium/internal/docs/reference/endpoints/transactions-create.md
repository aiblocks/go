...
clientData:
  laboratoryUrl: https://www.aiblocks.io/laboratory/#explorer?resource=transactions&endpoint=create
replacement: https://developers.aiblocks.io/api/resources/transactions/post/
---

Posts a new [transaction](../resources/transaction.md) to the AiBlocks Network.
...
AiBlocks Network's ledger is a [complicated and asynchronous
process](https://www.aiblocks.io/developers/learn/concepts/transactions.html#life-cycle).
To reduce the complexity, millennium manages these asynchronous processes for the
client and will wait to hear results from the AiBlocks Network before returning
an HTTP response to a client.

Transaction submission to millennium aims to be
[idempotent](https://en.wikipedia.org/wiki/Idempotence#Computer_science_meaning):
a client can submit a given transaction to millennium more than once and millennium
will behave the same each time.  If the transaction has already been
successfully applied to the ledger, millennium will simply return the saved result
...
Information about [building transactions](https://www.aiblocks.io/developers/js-aiblocks-base/reference/building-transactions.html) in JavaScript.

### Timeout

If you are encountering this error it means that either:

* Millennium has not received a confirmation from the Core server that the transaction you are trying to submit to the network was included in a ledger in a timely manner or:
* Millennium has not sent a response to a reverse-proxy before in a specified time.
