...
clientData:
  laboratoryUrl: https://www.aiblocks.io/laboratory/#explorer?resource=operations&endpoint=all
...
and Millennium is ingesting failed transactions.
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen as operations are processed in the AiBlocks network.
If called in streaming mode Millennium will start at the earliest known operation unless a `cursor` is set. In that case it will start from the `cursor`. You can also set `cursor` value to `now` to only stream operations created since your request time.
...
```sh
curl "https://millennium-testnet.aiblocks.io/operations?limit=200&order=desc"
```
