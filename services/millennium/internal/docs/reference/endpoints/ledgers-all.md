...
title: All Ledgers
clientData:
  laboratoryUrl: https://www.aiblocks.io/laboratory/#explorer?resource=ledgers&endpoint=all
replacement: https://developers.aiblocks.io/api/resources/ledgers/
---

This endpoint represents all [ledgers](../resources/ledger.md).
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to get notifications as ledgers are closed by the AiBlocks network.
If called in streaming mode Millennium will start at the earliest known ledger unless a `cursor` is set. In that case it will start from the `cursor`. You can also set `cursor` value to `now` to only stream ledgers created since your request time.
...
curl "https://millennium-testnet.aiblocks.io/ledgers?limit=200&order=desc"
```
