...
func TestProtocol14StateVerifier(t *testing.T) {
	...
	itest.RunMillenniumCLICommand("expingest", "trigger-state-rebuild")
...
func waitForStateVerifications(itest *integration.Test, count int) bool {
	...
	for i := 0; i < 120; i++ {
		...
		stateInvalid := strings.Contains(metrics, "millennium_ingest_state_invalid 1")
		assert.False(t, stateInvalid, "State is invalid!")

		notVerifiedYet := strings.Contains(
			metrics,
			fmt.Sprintf("millennium_ingest_state_verify_duration_seconds_count %d", count-1),
		)
		if notVerifiedYet {
			time.Sleep(time.Second)
			continue
