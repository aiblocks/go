...
func TestOperationIncludeTransactions(t *testing.T) {
	...
	withoutTransactionsQuery := (&Q{tt.MillenniumSession()}).Operations().
		ForAccount(accountID)

	var expectedTransactions []Transaction
	err = (&Q{tt.MillenniumSession()}).Transactions().ForAccount(accountID).Select(&expectedTransactions)
	tt.Assert.NoError(err)
