...
import (
	...
	supportTime "github.com/aiblocks/go/support/time"
...
func TestTradeQueries(t *testing.T) {
	...
	delo, err := q.GetAssetID(xdr.MustNewNativeAsset())
	tt.Require.NoError(err)
	...
	err = q.TradesForAssetPair(delo, assetUSD).Page(db2.MustPageQuery("", false, "asc", 100)).Select(&trades)
	tt.Require.NoError(err)
	...
	// reverse assets
	err = q.TradesForAssetPair(assetUSD, delo).Page(db2.MustPageQuery("", false, "asc", 100)).Select(&trades)
	tt.Require.NoError(err)
	tt.Assert.Len(trades, 1)
