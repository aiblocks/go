...
import (
	...
	"github.com/aiblocks/go/services/millennium/internal/ingest/processors"
...
type millenniumTransactionProcessor interface {
...
) millenniumTransactionProcessor {
...
// checkIfProtocolVersionSupported checks if this Millennium version supports the
...
func (s *ProcessorRunner) checkIfProtocolVersionSupported(ledgerSequence uint32) error {
	...
	if ledgerVersion > MaxSupportedProtocolVersion {
		return fmt.Errorf(
			"This Millennium version does not support protocol version %d. "+
				"The latest supported protocol version is %d. Please upgrade to the latest Millennium version.",
			ledgerVersion,
...
// matches the one in corresponding ledger header in aiblocks-core backend.
// This gives you full security if data in aiblocks-core backend can be trusted
...
func (s *ProcessorRunner) validateBucketList(ledgerSequence uint32) error {
	...
	if !exists {
		return fmt.Errorf(
			"cannot validate bucket hash list. Checkpoint ledger (%d) must exist in AiBlocks-Core database.",
			ledgerSequence,
		)
	}
...
func (s *ProcessorRunner) runChangeProcessorOnLedger(
	changeProcessor millenniumChangeProcessor, ledger uint32,
) error {
	var changeReader io.ChangeReader
	var err error
