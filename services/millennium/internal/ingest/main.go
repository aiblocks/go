...
const (
	...
	// MaxDBConnections is the size of the postgres connection pool dedicated to Millennium ingestion:
	//  * Ledger ingestion,
	//  * State verifications,
	//  * Metrics updates.
	MaxDBConnections = 3

	defaultCoreCursorName           = "MILLENNIUM"
...
type aiblocksCoreClient interface {
...
type system struct {
	...
	aiblocksCoreClient aiblocksCoreClient
...
func NewSystem(config Config) (System, error) {
	...
	if config.EnableCaptiveCore {
		if len(config.RemoteCaptiveCoreURL) > 0 {
			...
			ledgerBackend, err = ledgerbackend.NewCaptive(
				config.AiBlocksCoreBinaryPath,
				config.AiBlocksCoreConfigPath,
	...
	system := &system{
		...
		aiblocksCoreClient: &aiblockscore.Client{
			URL: config.AiBlocksCoreURL,
		},
...
func (s *system) initMetrics() {
	s.metrics.LedgerIngestionDuration = prometheus.NewSummary(prometheus.SummaryOpts{
		Namespace: "millennium", Subsystem: "ingest", Name: "ledger_ingestion_duration_seconds",
	...
	s.metrics.StateVerifyDuration = prometheus.NewSummary(prometheus.SummaryOpts{
		Namespace: "millennium", Subsystem: "ingest", Name: "state_verify_duration_seconds",
		Help: "state verification durations, sliding window = 10m",
	})

	s.metrics.StateInvalidGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			Namespace: "millennium", Subsystem: "ingest", Name: "state_invalid",
	...
	s.metrics.LedgerStatsCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "millennium", Subsystem: "ingest", Name: "ledger_stats_total",
...
// that means that Millennium ingestion can be running on multiple machines and
...
// of order book graph in memory of each Millennium instance. To solve this:
// * For state init:
	...
	for {
		...
		if err != nil {
			...
			if isCancelledError(err) {
				// We only expect context.Canceled errors to occur when millennium is shutting down
...
func (s *system) updateCursor(ledgerSequence uint32) error {
	if s.aiblocksCoreClient == nil || s.config.EnableCaptiveCore {
		return nil
	}

	cursor := defaultCoreCursorName
	if s.config.AiBlocksCoreCursor != "" {
		cursor = s.config.AiBlocksCoreCursor
	}

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	err := s.aiblocksCoreClient.SetCursor(ctx, cursor, int32(ledgerSequence))
	if err != nil {
		return errors.Wrap(err, "Setting aiblocks-core cursor failed")
	}
