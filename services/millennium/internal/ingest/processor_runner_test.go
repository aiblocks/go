...
func TestProcessorRunnerRunHistoryArchiveIngestionProtocolVersionNotSupported(t *testing.T) {
	...
	assert.EqualError(t, err, "Error while checking for supported protocol version: This Millennium version does not support protocol version 200. The latest supported protocol version is 15. Please upgrade to the latest Millennium version.")
}
