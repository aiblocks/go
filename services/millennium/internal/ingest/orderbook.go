...
// offers in the Millennium DB. Any offers which are created, modified, or removed
// from the Millennium DB during ingestion will be applied to the in memory order book
...
func NewOrderBookStream(historyQ history.IngestionQ, graph orderbook.OBGraph) *OrderBookStream {
	return &OrderBookStream{
		...
		LatestLedgerGauge: prometheus.NewGauge(prometheus.GaugeOpts{
			Namespace: "millennium", Subsystem: "order_book_stream", Name: "latest_ledger",
		}),
...
func (o *OrderBookStream) getIngestionStatus() (ingestionStatus, error) {
	...
	status.HistoryConsistentWithState = (status.LastIngestedLedger == lastHistoryLedger) ||
		...
		// Then, on the next iteration, we ingest TX Meta from AiBlocks Core. So there is a brief
...
func (o *OrderBookStream) update(status ingestionStatus) (bool, error) {
	...
	if reset {
		...
		// wait until offers in millennium db is valid before populating order book graph
...
// Update will query the Millennium DB for offers which have been created, removed, or updated since the
...
// Millennium DB (assuming no error is returned).
func (o *OrderBookStream) Update() error {
	...
	// add 15 minute jitter so that not all millennium nodes are calling
