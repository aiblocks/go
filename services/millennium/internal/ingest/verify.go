...
import (
	...
	"github.com/aiblocks/go/ingest/verify"
	"github.com/aiblocks/go/services/millennium/internal/db2"
...
func (s *system) verifyState(verifyAgainstLatestCheckpoint bool) error {
	...
	if verifyAgainstLatestCheckpoint {
		...
		// Millennium is catching up. It doesn't make sense to verify old ledgers as
		...
		localLog.Info("Starting state verification. Waiting 40 seconds for aiblocks-core to publish HAS...")
		select {
		case <-s.ctx.Done():
			localLog.Info("State verifier shut down...")
			return nil
		case <-time.After(40 * time.Second):
			// Wait for aiblocks-core to publish HAS
		}
	}
