...
import (
	...
	"github.com/aiblocks/go/services/millennium/internal/toid"
...
var (
	...
	// millennium's most recently ingested ledger
	ErrReingestRangeConflict = errors.New("reingest range overlaps with millennium ingestion")
)
...
func (b buildState) run(s *system) (transition, error) {
	...
	if err = s.updateCursor(b.checkpointLedger - 1); err != nil {
		...
		log.WithError(err).Warn("error updating aiblocks-core cursor")
	}
...
func (r resumeState) run(s *system) (transition, error) {
	...
	if err != nil {
		return retryResume(r), errors.Wrap(err, "Error getting lastest ledger in aiblocks-core")
	}
	...
	if latestLedgerCore < ingestLedger {
		...
		if err != nil {
			return retryResume(r), errors.Wrap(err, "Error fast-forwarding to the latest ledger in aiblocks-core")
		}

		if latestLedgerCore == ingestLedger-1 {
			logger.Info("Waiting for ledger to be available in aiblocks-core")
		} else {
...
func (h historyRangeState) run(s *system) (transition, error) {
	...
	// lastHistoryLedger. Otherwise, some other millennium node has
