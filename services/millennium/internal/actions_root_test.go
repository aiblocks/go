...
func TestRootAction(t *testing.T) {
	...
	ht.App.config.AiBlocksCoreURL = server.URL
	ht.App.config.NetworkPassphrase = "test"
	ht.App.UpdateAiBlocksCoreInfo()
	ht.App.UpdateLedgerState()

	w := ht.Get("/")

	if ht.Assert.Equal(200, w.Code) {
		var actual millennium.Root
		err := json.Unmarshal(w.Body.Bytes(), &actual)
		ht.Require.NoError(err)
		ht.Assert.Equal("devel", actual.MillenniumVersion)
		ht.Assert.Equal("test-core", actual.AiBlocksCoreVersion)
