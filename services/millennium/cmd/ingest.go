...
var ingestStressTestCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if config.EnableCaptiveCoreIngestion {
			ingestConfig.AiBlocksCoreBinaryPath = config.AiBlocksCoreBinaryPath
...
var ingestTriggerStateRebuildCmd = &cobra.Command{
	...
	Short: "updates a database to trigger state rebuild, state will be rebuilt by a running Millennium instance, DO NOT RUN production DB, some endpoints will be unavailable until state is rebuilt",
	Run: func(cmd *cobra.Command, args []string) {
		millennium.ApplyFlags(config, flags)
		...
		historyQ := &history.Q{millenniumSession}
