# `aiblocks/expingest-verify-range`

This docker image allows running multiple instances of `millennium expingest verify-command` on a single machine or running it in [AWS Batch](https://aws.amazon.com/batch/).
...
* In "Job definition" set vCPUs to 2 and Memory to 4096. This represents the `c5.large` instances Millennium should be using.
