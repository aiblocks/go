...
// Minion contains a AiBlocks channel account and Go channels to communicate with friendbot.
type Minion struct {
	Account         Account
	Keypair         *keypair.Full
	BotAccount      txnbuild.Account
	BotKeypair      *keypair.Full
	Millennium         *millenniumclient.Client
	Network         string
	StartingBalance string
	BaseFee         int64

	// Mockable functions
	SubmitTransaction    func(minion *Minion, hclient *millenniumclient.Client, tx string) (*hProtocol.Transaction, error)
	CheckSequenceRefresh func(minion *Minion, hclient *millenniumclient.Client) error
...
func (minion *Minion) Run(destAddress string, resultChan chan SubmitResult) {
	err := minion.CheckSequenceRefresh(minion, minion.Millennium)
	if err != nil {
	...
	succ, err := minion.SubmitTransaction(minion, minion.Millennium, txStr)
	resultChan <- SubmitResult{
...
func SubmitTransaction(minion *Minion, hclient *millenniumclient.Client, tx string) (*hProtocol.Transaction, error) {
	result, err := hclient.SubmitTransactionXDR(tx)
	if err != nil {
		errStr := "submitting tx to millennium"
		switch e := err.(type) {
		case *millenniumclient.Error:
			minion.checkHandleBadSequence(e)
			resStr, resErr := e.ResultString()
			if resErr != nil {
				errStr += ": error getting millennium error code: " + resErr.Error()
			} else if resStr == createAccountAlreadyExistXDR {
				return nil, errors.Wrap(ErrAccountExists, errStr)
			} else {
				errStr += ": millennium error string: " + resStr
			}
...
func CheckSequenceRefresh(minion *Minion, hclient *millenniumclient.Client) error {
...
func (minion *Minion) checkHandleBadSequence(err *millenniumclient.Error) {
