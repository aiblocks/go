...
func calcOrderbookStats(obStats *OrderbookStats, summary hProtocol.OrderBookSummary) error {
	...
	for _, ask := range summary.Asks {
		...
		// On Millennium, Ask prices are in units of counter, but
		// amount is in units of base. Therefore, real amount = amount * price
		// See: https://github.com/aiblocks/go/issues/612
...
// createOrderbookRequest generates a millenniumclient.OrderBookRequest based on the base
// and counter asset parameters provided
func createOrderbookRequest(bType, bCode, bIssuer, cType, cCode, cIssuer string) millenniumclient.OrderBookRequest {
	r := millenniumclient.OrderBookRequest{
		SellingAssetType: millenniumclient.AssetType(bType),
		BuyingAssetType:  millenniumclient.AssetType(cType),
		// NOTE (Alex C, 2019-05-02):
		// Orderbook requests are currently not paginated on Millennium.
		// This limit has been added to ensure we capture at least 200
		// orderbook entries once pagination is added.
		Limit: 200,
	}

	// The Millennium API requires *AssetCode and *AssetIssuer fields to be empty
	// when an Asset is native. As we store "DLO" as the asset code for native,
	// we should only add Code and Issuer info in case we're dealing with
	// non-native assets.
	// See: https://www.aiblocks.io/developers/millennium/reference/endpoints/orderbook-details.html
	if bType != string(millenniumclient.AssetTypeNative) {
		r.SellingAssetCode = bCode
		r.SellingAssetIssuer = bIssuer
	}
	if cType != string(millenniumclient.AssetTypeNative) {
