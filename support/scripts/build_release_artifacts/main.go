...
// This is a build script that Travis uses to build AiBlocks release packages.
...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/aiblocks/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/aiblocks/go/support/app.version=%s", version)
...
// Tags must be of the form `NAME-vSEMVER`, such as `millennium-v1.0.0` to be
