// Package network contains functions that deal with aiblocks network passphrases
// and IDs.
package network
...
import (
	...
	"github.com/aiblocks/go/hash"
	"github.com/aiblocks/go/support/errors"
	"github.com/aiblocks/go/xdr"
)

const (
	// PublicNetworkPassphrase is the pass phrase used for every transaction intended for the public aiblocks network
	PublicNetworkPassphrase = "Public Global AiBlocks Network ; September 2015"
...
// The resulting hash is the value that can be signed by aiblocks secret key to
// authorize the transaction identified by the hash to aiblocks validators.
...
// resulting hash is the value that can be signed by aiblocks secret key to
// authorize the transaction identified by the hash to aiblocks validators.
