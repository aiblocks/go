...
func PrepareRangePollInterval(d time.Duration) RemoteCaptiveOption {
	return func(c *RemoteCaptiveAiBlocksCore) {
...
func NewRemoteCaptive(captiveCoreURL string, options ...RemoteCaptiveOption) (RemoteCaptiveAiBlocksCore, error) {
	...
	if err != nil {
		return RemoteCaptiveAiBlocksCore{}, errors.Wrap(err, "unparseable url")
	}

	client := RemoteCaptiveAiBlocksCore{
...
func (c RemoteCaptiveAiBlocksCore) createContext() context.Context {
	c.lock.Lock()
	defer c.lock.Unlock()
