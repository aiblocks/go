...
// Volume stores volume of a various base pair in both DLO and USD.
...
func initVolumes(cfg Config, c trackerClient, watchedTPs []prometheusWatchedTP) map[string][]volumeHist {
	dloReq := mustCreateDloPriceRequest()
	dloPriceHist, err := getDloPriceHistory(dloReq)
	if err != nil {
		fmt.Printf("got error when getting dlo price history: %s\n", err)
	}
	...
	cRes := time.Duration(res*1000) * time.Millisecond // millennium request must be in milliseconds

	for i, wtp := range watchedTPs {
		// TODO: Calculate volume for assets with non-native counter.
		if wtp.TradePair.SellingAsset.Code != "DLO" && wtp.TradePair.SellingAsset.IssuerAddress != "" {
			continue
		}
		...
		volumeHist, err := constructVolumeHistory(records, dloPriceHist, trueAssetUsdPrice, start, end, res)
		if err != nil {
...
func updateVolume(cfg Config, c trackerClient, watchedTPsPtr *[]prometheusWatchedTP, volumeHistMap map[string][]volumeHist) {
	req := mustCreateDloPriceRequest()
	historyUnit := time.Duration(15 * 60 * time.Second) // length of each individual unit of volume history
	cRes := time.Duration(60*1000) * time.Millisecond   // millennium client requests have a 1 minute resolution, in milliseconds
	...
	for {
		...
		dloUsdPrice, err := getLatestDloPrice(req)
		if err != nil {
		...
		for i, wtp := range watchedTPs {
			...
			if err != nil {
				fmt.Printf("got error aggregating dlo volume for pair %s\n: %s", tps, err)
			}
			...
			latestVolume := volumeHist{
				...
				counterVolumeBaseAsset: counterVolume * dloUsdPrice * trueAssetUsdPrice,
				counterVolumeUsd:       counterVolume * dloUsdPrice,
				numTrades:              numTrades,
			}
...
func constructVolumeHistory(tas []hProtocol.TradeAggregation, dloPrices []dloPrice, assetPrice float64, start, end time.Time, res int) ([]volumeHist, error) {
	if len(dloPrices) < 2 {
		return []volumeHist{}, fmt.Errorf("mis-formed dlo price history from aiblocks expert")
	}
	...
	for currEnd.After(start) {
		...
		cets := currEnd.Unix()
		csts := cets - int64(res)
		priceIdx = findTimestampPriceIndex(csts, dloPrices, priceIdx)

		weightedDloUsdPrice, err := calcWeightedPrice(csts, priceIdx, dloPrices)
		if err != nil {
		...
		currVolume := volumeHist{
			...
			counterVolumeBaseAsset: currCounterVolume * weightedDloUsdPrice * assetPrice,
			counterVolumeUsd:       weightedDloUsdPrice * currCounterVolume,
		}
...
func findTimestampPriceIndex(timestamp int64, prices []dloPrice, startIndex int) int {
	index := startIndex
	if index < 0 {
...
func calcWeightedPrice(timestamp int64, startIndex int, prices []dloPrice) (float64, error) {
