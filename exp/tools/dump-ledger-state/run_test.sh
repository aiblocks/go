...
if [ -z ${TESTNET+x} ]; then
    ...
    aiblocks-core --conf ./aiblocks-core.cfg catchup $LATEST_LEDGER/1
else
    echo "Catch up from testnet"
    aiblocks-core --conf ./aiblocks-core-testnet.cfg catchup $LATEST_LEDGER/1
fi

echo "Dumping state from aiblocks-core..."
./dump_core_db.sh
echo "State dumped..."
