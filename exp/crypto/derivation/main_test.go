...
func ExampleDeriveFromPath() {
	...
	key, err := DeriveForPath(AiBlocksPrimaryAccountPath, seed)
	if err != nil {
		panic(err)
	}
...
func ExampleDeriveMultipleKeys() {
	...
	for i := 0; i < 10; i++ {
		path := fmt.Sprintf(AiBlocksAccountPathFormat, i)
...
func ExampleDeriveMultipleKeysFaster() {
	seed, _ := hex.DecodeString("000102030405060708090a0b0c0d0e0f")
	mainKey, err := DeriveForPath(AiBlocksAccountPrefix, seed)
	if err != nil {
		panic(err)
	}
	...
	for i := uint32(0); i < 10; i++ {
		...
		fmt.Println(fmt.Sprintf(AiBlocksAccountPathFormat, i), kp.Seed(), kp.Address())
	}
...
func BenchmarkDerive(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := DeriveForPath(AiBlocksPrimaryAccountPath, seed)
		if err != nil {
			panic(err)
		}
	}
