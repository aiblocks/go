...
// Account represents a AiBlocks account for this demo.
type Account struct {
...
func Reset(client *millenniumclient.Client, keys []Account) {
	...
	for _, k := range keys {
		...
		// See https://www.aiblocks.io/developers/guides/concepts/ledger.html#ledger-entries
...
// first account.
func Initialise(client *millenniumclient.Client, keys []Account) {
...
// from Millennium. This code demonstrates how to retrieve and inspect the error.
func TXError(client *millenniumclient.Client, keys []Account) {
	keys = loadAccounts(client, keys)
...
func loadAccounts(client *millenniumclient.Client, accounts []Account) []Account {
...
// loadAccount is an example of how to get an account's details from Millennium.
func loadAccount(client *millenniumclient.Client, address string) *hProtocol.Account {
	accountRequest := millenniumclient.AccountRequest{AccountID: address}
	millenniumSourceAccount, err := client.AccountDetail(accountRequest)
	if err != nil {
		dieIfError(fmt.Sprintf("couldn't get account detail for %s", address), err)
	}

	return &millenniumSourceAccount
}

func submit(client *millenniumclient.Client, txeBase64 string) (resp hProtocol.Transaction) {
	resp, err := client.SubmitTransactionXDR(txeBase64)
	if err != nil {
		hError := err.(*millenniumclient.Error)
		err = printMillenniumError(hError)
		dieIfError("couldn't print Millennium eror", err)
		os.Exit(1)
	}
...
// printMillenniumError is an example of how to inspect the error returned from Millennium.
func printMillenniumError(hError *millenniumclient.Error) error {
