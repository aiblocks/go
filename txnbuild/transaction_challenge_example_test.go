...
var millenniumClient = func() millenniumclient.ClientInterface {
	client := &millenniumclient.MockClient{}
	client.
		On("AccountDetail", millenniumclient.AccountRequest{AccountID: clientAccount.Address()}).
		Return(
			millennium.Account{
				Thresholds: millennium.AccountThresholds{LowThreshold: 1, MedThreshold: 10, HighThreshold: 100},
				Signers: []millennium.Signer{
...
func ExampleVerifyChallengeTxThreshold() {
	...
	// Server verifies signed challenge transaction
	{
		...
		millenniumClientAccount, err := millenniumClient.AccountDetail(millenniumclient.AccountRequest{AccountID: txClientAccountID})
		if millenniumclient.IsNotFoundError(err) {
			clientAccountExists = false
		...
		if clientAccountExists {
			...
			signerSummary := millenniumClientAccount.SignerSummary()

			// Server chooses the threshold to require: low, med or high
			threshold := txnbuild.Threshold(millenniumClientAccount.Thresholds.MedThreshold)
