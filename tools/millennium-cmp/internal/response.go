...
// differences in responses between two Millennium version.
//
// Let's say that next Millennium version adds a new bool field:
...
var removeRegexps = []*regexp.Regexp{
	...
	// Removes joined transaction (join=transactions) added in Millennium 0.19.0.
	...
	// Removes last_modified_time field, introduced in millennium 1.3.0
...
var replaceRegexps = []replace{
	...
	{regexp.MustCompile(
		// fee_charged is a string since millennium 1.3.0
		`"fee_charged":( ?)([\d]+),`),
		`"fee_charged":${1}"${2}",`,
	},
	{regexp.MustCompile(
		// max_fee is a string since millennium 1.3.0
		`"max_fee":( ?)([\d]+),`),
		`"max_fee":${1}"${2}",`,
	},
	// Removes trailing SSE data, fixed in millennium 1.7.0
	{regexp.MustCompile(
		`\nretry:.*\nevent:.*\ndata:.*\n`),
		``,
	},
...
func NewResponse(domain, path string, stream bool) *Response {
	...
	// Remove Millennium URL from the _links
