...
func checkPaths(paths []string) {
	for _, path := range paths {
		a := cmp.NewResponse(millenniumBase, path, false)
		b := cmp.NewResponse(millenniumTest, path, false)
...
func getAllPagesPaths(page string) []string {
	...
	for {
		resp, err := http.Get(millenniumBase + page)
		if err != nil {
			panic(err)
		}
		...
		page = strings.Replace(page, millenniumBase, "", -1)
		...
		// no extra objects in the Millennium we are testing.
