...
import (
	...
	client "github.com/aiblocks/go/clients/millenniumclient"
	protocol "github.com/aiblocks/go/protocols/millennium"
	"github.com/aiblocks/go/support/errors"
	slog "github.com/aiblocks/go/support/log"
	cmp "github.com/aiblocks/go/tools/millennium-cmp/internal"
)
...
// CLI params
var (
	millenniumBase           string
	millenniumTest           string
...
var rootCmd = &cobra.Command{
	Use:   "millennium-cmp",
	Short: "millennium-cmp compares two millennium servers' responses",
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&millenniumBase, "base", "b", "", "URL of the base/old version Millennium server")
	rootCmd.PersistentFlags().StringVarP(&millenniumTest, "test", "t", "", "URL of the test/new version Millennium server")
...
func run(cmd *cobra.Command) {
	if millenniumBase == "" || millenniumTest == "" {
	...
	ledger := getLatestLedger(millenniumBase)
	...
	outputDir := fmt.Sprintf("%s/millennium-cmp-diff/%s", pwd, time.Now().Format(timeFormat))
	...
	for {
		...
		go func() {
			...
			go func() {
				a = cmp.NewResponse(millenniumBase, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			go func() {
				b = cmp.NewResponse(millenniumTest, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			...
			// older Millennium versions don't send this header.
...
func getLatestLedger(url string) protocol.Ledger {
	millennium := client.Client{
		MillenniumURL: url,
		HTTP:       http.DefaultClient,
	}

	ledgers, err := millennium.Ledgers(client.LedgerRequest{
...
func addPathsFromResponse(a *cmp.Response, level int) {
	...
	for _, newPath := range newPaths {
		...
		// without cursor. There will always be a diff if Millennium started
