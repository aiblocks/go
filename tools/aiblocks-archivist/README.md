...
This is a small tool, written in Go, for working with `aiblocks-core` history archives directly.
It is a standalone tool that does not require `aiblocks-core`, or any other programs.

It is much smaller and simpler than `aiblocks-core`, and is intended only for archive-maintenance tasks.
...
```
$ go install github.com/aiblocks/go/tools/aiblocks-archivist
```

## Usage

```
inspect aiblocks history archive

Usage:
  aiblocks-archivist [flags]
  aiblocks-archivist [command]
...
Flags:
  ...
  -h, --help              help for aiblocks-archivist
...
Use "aiblocks-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `aiblocks-core`, `aiblocks-archivist` does not run subprocesses to access history archives;
...
`aiblocks-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ aiblocks-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
$ aiblocks-archivist status --s3endpoint https://storage.googleapis.com s3://google-storage-bucketname
``` 
...
```
$ aiblocks-archivist status http://s3-eu-west-1.amazonaws.com/history.aiblocks.io/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.aiblocks.io/prd/core-testnet/core_testnet_001
...
```
$ aiblocks-archivist mirror http://s3-eu-west-1.amazonaws.com/history.aiblocks.io/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.aiblocks.io/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ aiblocks-archivist mirror --recent http://history.aiblocks.io/prd/core-live/core_live_001 file://local-archive

2019/08/21 13:53:15 mirroring http://history.aiblocks.io/prd/core-live/core_live_001 -> file://local-archive
...
```
$ aiblocks-archivist --last 1024 mirror http://history.aiblocks.io/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.aiblocks.io/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ aiblocks-archivist scan file://local-archive
...
```
$ aiblocks-archivist --last 4096 scan file://local-archive
...
$ aiblocks-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ aiblocks-archivist --verify --last 4096 scan file://local-archive
...
$ aiblocks-archivist repair file://local-archive file://broken-archive
...
```
 aiblocks-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
