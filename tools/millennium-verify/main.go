...
var millenniumURL string
var startSequence uint32
var count uint

func init() {
	rootCmd.PersistentFlags().StringVarP(&millenniumURL, "url", "u", "", "Millennium server URL")
...
var rootCmd = &cobra.Command{
	Use:   "millennium-verify",
	Short: "tool to check millennium data consistency",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("%s: Checking %d ledgers starting from cursor \"%s\"\n\n", millenniumURL, count, ledgerCursor)

		for {
			ledgersPage, err := client.Ledgers(millenniumclient.LedgerRequest{
				Limit:  200,
			...
			for _, ledger := range ledgersPage.Embedded.Records {
				...
				transactionsPage, err := client.Transactions(millenniumclient.TransactionRequest{
				...
				for _, transaction := range transactionsPage.Embedded.Records {
					...
					go func(transaction protocol.Transaction) {
						...
						operationsPage, err := client.Operations(millenniumclient.OperationRequest{
