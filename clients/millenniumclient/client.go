...
// sendRequest builds the URL for the given millennium request and sends the url to a millennium server
func (c *Client) sendRequest(hr MillenniumRequest, resp interface{}) (err error) {
	endpoint, err := hr.BuildURL()
	if err != nil {
		return
	}

	c.MillenniumURL = c.fixMillenniumURL()
	_, ok := hr.(submitRequest)
	if ok {
		return c.sendRequestURL(c.MillenniumURL+endpoint, "post", resp)
	}

	return c.sendRequestURL(c.MillenniumURL+endpoint, "get", resp)
}

// checkMemoRequired implements a memo required check as defined in
// https://github.com/aiblocks/aiblocks-protocol/blob/master/ecosystem/sep-0029.md
func (c *Client) checkMemoRequired(transaction *txnbuild.Transaction) error {
	...
	for i, op := range transaction.Operations() {
		...
		if err != nil {
			millenniumError := GetError(err)

			if millenniumError == nil || millenniumError.Response.StatusCode != 404 {
				return err
			}
...
// sendRequestURL sends a url to a millennium server.
// It can be used for requests that do not implement the MillenniumRequest interface.
func (c *Client) sendRequestURL(requestURL string, method string, a interface{}) (err error) {
	...
	c.setDefaultClient()
	if c.millenniumTimeout == 0 {
		c.millenniumTimeout = MillenniumTimeout
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*c.millenniumTimeout)
...
// stream handles connections to endpoints that support streaming on a millennium server
			...
			for {
				...
				if err != nil {
					if err == io.EOF || err == io.ErrUnexpectedEOF {
						...
						//   happen in Millennium as it always sends an empty line after each event.
						// - The stream was closed by the server/proxy because the connection was idle.
						//
						// In the former case, that (again) should never happen in Millennium, we need to
						// check if there are any events we need to decode. We do this in the `if`
						// statement below just in case if Millennium behaviour changes in a future.
						//
						// From spec:
...
func (c *Client) setClientAppHeaders(req *http.Request) {
	req.Header.Set("X-Client-Name", "go-aiblocks-sdk")
...
// fixMillenniumURL strips all slashes(/) at the end of MillenniumURL if any, then adds a single slash
func (c *Client) fixMillenniumURL() string {
	return strings.TrimRight(c.MillenniumURL, "/") + "/"
}

// SetMillenniumTimeout allows users to set the timeout before a millennium request is cancelled.
// The timeout is specified as a time.Duration which is in nanoseconds.
func (c *Client) SetMillenniumTimeout(t time.Duration) *Client {
	c.millenniumTimeout = t
	return c
}

// MillenniumTimeout returns the current timeout for a millennium client
func (c *Client) MillenniumTimeout() time.Duration {
	return c.millenniumTimeout
}

// Accounts returns accounts who have a given signer or
// have a trustline to an asset.
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/accounts.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/accounts-single.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/data-for-account.html
...
// Effects returns effects(https://www.aiblocks.io/developers/millennium/reference/resources/effect.html)
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/assets-all.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/ledgers-all.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/ledgers-single.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/fee-stats.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/offers-for-account.html
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/offer-details.html
...
// Operations returns aiblocks operations (https://www.aiblocks.io/developers/millennium/reference/resources/operation.html)
...
// OperationDetail returns a single aiblocks operation for a given operation id
// See https://www.aiblocks.io/developers/millennium/reference/resources/operation.html
func (c *Client) OperationDetail(id string) (ops operations.Operation, err error) {
	...
	if err != nil {
		return ops, errors.Wrap(err, "sending request to millennium")
	}
...
// SubmitTransactionXDR submits a transaction represented as a base64 XDR string to the network. err can be either error object or millennium.Error object.
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/transactions-create.html
...
// error object or a millennium.Error object.
//
// This function will always check if the destination account requires a memo in the transaction as
// defined in SEP0029: https://github.com/aiblocks/aiblocks-protocol/blob/master/ecosystem/sep-0029.md
//
...
// you to pass SubmitTxOpts. err can be either an error object or a millennium.Error object.
//
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/transactions-create.html
...
// Transactions returns aiblocks transactions (https://www.aiblocks.io/developers/millennium/reference/resources/transaction.html)
...
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/transactions-single.html
...
// OrderBook returns the orderbook for an asset pair (https://www.aiblocks.io/developers/millennium/reference/resources/orderbook.html)
func (c *Client) OrderBook(request OrderBookRequest) (obs hProtocol.OrderBookSummary, err error) {
	err = c.sendRequest(request, &obs)
	return
}

// Paths returns the available paths to make a strict receive path payment. See https://www.aiblocks.io/developers/millennium/reference/endpoints/path-finding-strict-receive.html
...
// StrictReceivePaths returns the available paths to make a strict receive path payment. See https://www.aiblocks.io/developers/millennium/reference/endpoints/path-finding-strict-receive.html
func (c *Client) StrictReceivePaths(request PathsRequest) (paths hProtocol.PathsPage, err error) {
	err = c.sendRequest(request, &paths)
	return
}

// StrictSendPaths returns the available paths to make a strict send path payment. See https://www.aiblocks.io/developers/millennium/reference/endpoints/path-finding-strict-send.html
func (c *Client) StrictSendPaths(request StrictSendPathsRequest) (paths hProtocol.PathsPage, err error) {
	err = c.sendRequest(request, &paths)
	return
}

// Payments returns aiblocks account_merge, create_account, path payment and payment operations.
...
// Trades returns aiblocks trades (https://www.aiblocks.io/developers/millennium/reference/resources/trade.html)
...
// https://www.aiblocks.io/developers/guides/get-started/create-account.html for more information.
func (c *Client) Fund(addr string) (tx hProtocol.Transaction, err error) {
	...
	friendbotURL := fmt.Sprintf("%sfriendbot?addr=%s", c.fixMillenniumURL(), addr)
...
// TradeAggregations returns aiblocks trade aggregations (https://www.aiblocks.io/developers/millennium/reference/resources/trade_aggregation.html)
...
// StreamEffects streams millennium effects. It can be used to stream all effects or account specific effects.
...
// StreamOperations streams aiblocks operations. It can be used to stream all operations or operations
...
// StreamPayments streams aiblocks payments. It can be used to stream all payments or payments
...
// StreamOffers streams offers processed by the AiBlocks network for an account. Use context.WithCancel
...
// StreamLedgers streams aiblocks ledgers. It can be used to stream all ledgers. Use context.WithCancel
// to stop streaming or context.Background() if you want to stream indefinitely.
...
// FetchTimebounds provides timebounds for N seconds from now using the server time of the millennium instance.
...
func (c *Client) FetchTimebounds(seconds int64) (txnbuild.Timebounds, error) {
	serverURL, err := url.Parse(c.MillenniumURL)
	if err != nil {
		return txnbuild.Timebounds{}, errors.Wrap(err, "unable to parse millennium url")
	}
...
// Root loads the root endpoint of millennium
func (c *Client) Root() (root hProtocol.Root, err error) {
	err = c.sendRequestURL(c.fixMillenniumURL(), "get", &root)
	return
}
...
// ensure that the millennium client implements ClientInterface
