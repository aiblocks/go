...
// decodeResponse decodes the response from a request to a millennium server
func decodeResponse(resp *http.Response, object interface{}, hc *Client) (err error) {
	defer resp.Body.Close()
	decoder := json.NewDecoder(resp.Body)

	u, err := url.Parse(hc.MillenniumURL)
	if err != nil {
		return errors.Errorf("unable to parse the provided millennium url: %s", hc.MillenniumURL)
	}
	setCurrentServerTime(u.Hostname(), resp.Header["Date"], hc)

	if !(resp.StatusCode >= 200 && resp.StatusCode < 300) {
		millenniumError := &Error{
			Response: resp,
		}
		decodeError := decoder.Decode(&millenniumError.Problem)
		if decodeError != nil {
			return errors.Wrap(decodeError, "error decoding millennium.Problem")
		}
		return millenniumError
	}
...
// setCurrentServerTime saves the current time returned by a millennium server
...
// currentServerTime returns the current server time for a given millennium server
