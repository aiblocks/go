...
This library provides an interface to the AiBlocks Millennium service. It supports the building of Go applications on
top of the AiBlocks network (https://www.aiblocks.io/). Transactions may be constructed using the sister package to
this one, txnbuild (https://github.com/aiblocks/go/tree/master/txnbuild), and then submitted with this client to any
Millennium instance for processing onto the ledger. Together, these two libraries provide a complete AiBlocks SDK.
...
const (
	...
	// AssetTypeNative represents the asset type for AiBlocks Delos (DLO)
...
// Error struct contains the problem returned by Millennium
type Error struct {
...
var (
	...
	// MillenniumTimeout is the default number of nanoseconds before a request to millennium times out.
	MillenniumTimeout = 60 * time.Second
...
// HTTP represents the HTTP client that a millennium client uses to communicate
type HTTP interface {
...
// This handler is used when getting the time from a millennium server, which can be used to calculate
// transaction timebounds.
type UniversalTimeHandler func() int64

// Client struct contains data for creating a millennium client that connects to the aiblocks network.
type Client struct {
	// URL of Millennium server to connect
	MillenniumURL string

	// HTTP client to make requests with
	HTTP HTTP

	// AppName is the name of the application using the millenniumclient package
	AppName string

	// AppVersion is the version of the application using the millenniumclient package
	AppVersion     string
	millenniumTimeout time.Duration
	isTestNet      bool
...
// ClientInterface contains methods implemented by the millennium client
...
var DefaultTestNetClient = &Client{
	MillenniumURL:     "https://millennium-testnet.aiblocks.io/",
	HTTP:           http.DefaultClient,
	millenniumTimeout: MillenniumTimeout,
	isTestNet:      true,
}

// DefaultPublicNetClient is a default client to connect to public network.
var DefaultPublicNetClient = &Client{
	MillenniumURL:     "https://millennium.aiblocks.io/",
	HTTP:           http.DefaultClient,
	millenniumTimeout: MillenniumTimeout,
}

// MillenniumRequest contains methods implemented by request structs for millennium endpoints.
type MillenniumRequest interface {
	BuildURL() (string, error)
}

// AccountsRequest struct contains data for making requests to the accounts endpoint of a millennium server.
...
// AccountRequest struct contains data for making requests to the show account endpoint of a millennium server.
...
// EffectRequest struct contains data for getting effects from a millennium server.
...
// AssetRequest struct contains data for getting asset details from a millennium server.
...
// LedgerRequest struct contains data for getting ledger details from a millennium server.
// The query parameters (Order, Cursor and Limit) are optional. All or none can be set.
...
// OfferRequest struct contains data for getting offers made by an account from a millennium server.
// The query parameters (Order, Cursor and Limit) are optional. All or none can be set.
...
// OperationRequest struct contains data for getting operation details from a millennium server.
...
// TransactionRequest struct contains data for getting transaction details from a millennium server.
...
// OrderBookRequest struct contains data for getting the orderbook for an asset pair from a millennium server.
...
// PathsRequest struct contains data for getting available strict receive path payments from a millennium server.
// All the Destination related parameters are required and you need to include either
// SourceAccount or SourceAssets.
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/path-finding-strict-receive.html
...
// StrictSendPathsRequest struct contains data for getting available strict send path payments from a millennium server.
// All the Source related parameters are required and you need to include either
// DestinationAccount or DestinationAssets.
// See https://www.aiblocks.io/developers/millennium/reference/endpoints/path-finding-strict-send.html
...
// TradeRequest struct contains data for getting trade details from a millennium server.
...
// TradeAggregationRequest struct contains data for getting trade aggregations from a millennium server.
...
// ServerTimeRecord contains data for the current unix time of a millennium server instance, and the local time when it was recorded.
type ServerTimeRecord struct {
	ServerTime        int64
	LocalTimeRecorded int64
}

// ServerTimeMap holds the ServerTimeRecord for different millennium instances.
