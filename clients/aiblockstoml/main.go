...
// AiBlocksTomlMaxSize is the maximum size of aiblocks.toml file
const AiBlocksTomlMaxSize = 100 * 1024

// WellKnownPath represents the url path at which the aiblocks.toml file should
// exist to conform to the federation protocol.
const WellKnownPath = "/.well-known/aiblocks.toml"
...
// Client represents a client that is capable of resolving a AiBlocks.toml file
// using the internet.
type Client struct {
	// HTTP is the http client used when resolving a AiBlocks.toml file
	HTTP HTTP
...
type ClientInterface interface {
	GetAiBlocksToml(domain string) (*Response, error)
	GetAiBlocksTomlByAddress(addr string) (*Response, error)
}
...
// SEP-1 commit
// https://github.com/aiblocks/aiblocks-protocol/blob/f8993e36fa6b5b8bba1254c21c2174d250af4958/ecosystem/sep-0001.md
type Response struct {
	...
	MillenniumUrl                    string      `toml:"MILLENNIUM_URL"`
...
// GetAiBlocksToml returns aiblocks.toml file for a given domain
func GetAiBlocksToml(domain string) (*Response, error) {
	return DefaultClient.GetAiBlocksToml(domain)
}

// GetAiBlocksTomlByAddress returns aiblocks.toml file of a domain fetched from a
// given address
func GetAiBlocksTomlByAddress(addr string) (*Response, error) {
	return DefaultClient.GetAiBlocksTomlByAddress(addr)
}
