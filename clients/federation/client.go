...
// LookupByAddress performs a federated lookup following to the aiblocks
...
// "name" type is a legacy holdover from the legacy aiblocks network's federation
...
// LookupByAccountID performs a federated lookup following to the aiblocks
...
func (c *Client) LookupByAccountID(aid string) (*proto.IDResponse, error) {
	...
	domain, err := c.Millennium.HomeDomainForAccount(aid)
	if err != nil {
...
// ForwardRequest performs a federated lookup following to the aiblocks
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.AiBlocksTOML.GetAiBlocksToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get aiblocks.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("aiblocks.toml is missing federation server info")
	}
