...
// aiblocks-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with aiblocks-core.  If nil,
	// http.DefaultClient will be used.
	HTTP HTTP

	// URL of AiBlocks Core server to connect.
	URL string
}

// Info calls the `info` command on the connected aiblocks core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected aiblocks core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on aiblocks-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected aiblocks core with the provided envelope
...
// WaitForNetworkSync continually polls the connected aiblocks-core until it
...
// simpleGet returns a new GET request to the connected aiblocks-core using the
